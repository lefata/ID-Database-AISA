...
-   **Database Health Tool**: A one-click tool in the admin panel to verify and repair the database schema, tables, functions, and storage buckets, ensuring the application is correctly configured.
...
The setup script (`npm run db:setup`) will now automatically attempt to create the required public `avatars` bucket for you.

If the script fails with a permission error, you may need to create the bucket manually:
...
-   Click **New query** and paste the **entire** SQL script below into the editor, then click **Run**.
    ...
    -- 7. Create the schema verification and repair function
    ...
    BEGIN
        ...
        ELSE
            logs := logs || jsonb_build_object('status', 'info', 'step', 'Check Table: people', 'details', 'Table "people" already exists.');
        END IF;
        ...
        ELSE
            logs := logs || jsonb_build_object('status', 'info', 'step', 'Check Table: settings', 'details', 'Table "settings" already exists.');
        END IF;
        ...
        RETURNS BOOLEAN LANGUAGE SQL SECURITY DEFINER SET search_path = public AS $func$
            SELECT COALESCE((auth.jwt() -> 'user_metadata' ->> 'role') = 'admin', FALSE)
        $func$;
        logs := logs || jsonb_build_object('status', 'success', 'step', 'Create/Replace Function: is_admin', 'details', 'Function "is_admin" was created or updated.');
        ...
        DROP POLICY IF EXISTS "Allow admin update access on settings" ON public.settings;
        CREATE POLICY "Allow admin update access on settings" ON public.settings FOR UPDATE USING (is_admin()) WITH CHECK (is_admin());
        logs := logs || jsonb_build_object('status', 'success', 'step', 'Apply RLS Policies', 'details', 'RLS policies for "people" and "settings" have been applied/re-applied.');

        -- Trigger function and trigger
        CREATE OR REPLACE FUNCTION public.grant_first_user_admin()
        RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER AS $trigger_func$
        BEGIN
            IF (SELECT count(*) FROM auth.users) = 1 THEN
            UPDATE auth.users
            SET user_metadata = jsonb_set(COALESCE(user_metadata, '{}'::jsonb), '{role}', '"admin"'::jsonb), email_confirmed_at = NOW()
            WHERE id = NEW.id;
            END IF;
            RETURN NEW;
        END;
...
-   This script will create the storage bucket, verify your table schema, and seed the database with initial data if it's empty.
    ```bash
    npm run db:setup
    ```
