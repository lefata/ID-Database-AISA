                         ...
                         <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
                            ...
                            <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
                                ...
                                <div className="sm:col-span-2 grid grid-cols-2 gap-6">
                                    <div><label htmlFor="firstName" className="block text-sm font-medium text-slate-700">First Name</label><input type="text" id="firstName" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="e.g., Jane" className="mt-1 block w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-sky-500 focus:border-sky-500 sm:text-sm" /></div>
                                    <div><label htmlFor="lastName" className="block text-sm font-medium text-slate-700">Last Name</label><input type="text" id="lastName" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="e.g., Doe" className="mt-1 block w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-sky-500 focus:border-sky-500 sm:text-sm" /></div>
                                </div>
                                <div className="sm:col-span-2"><label htmlFor="roleOrClass" className="block text-sm font-medium text-slate-700">{category === PersonCategory.STAFF ? 'Job/Position' : (category === PersonCategory.STUDENT ? 'Class/Grade' : 'Association')}</label><input type="text" id="roleOrClass" placeholder={category === PersonCategory.STAFF ? 'e.g., Head Teacher' : category === PersonCategory.STUDENT ? 'e.g., Grade 5' : 'e.g., Parent of Leo Cole'} value={roleOrClass} onChange={(e) => setRoleOrClass(e.target.value)} className="mt-1 block w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-sky-500 focus:border-sky-500 sm:text-sm" /></div>
                            </div>
                                ...
                                <div className="md:col-span-3 mt-6 pt-6 border-t"><h3 className="text-xl font-semibold text-slate-800 mb-4">Guardians & Siblings</h3>
                                    <div className="space-y-6">
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-4">{newGuardians.map(g => 
                                                <div key={g.tempId} className="p-4 bg-slate-50 rounded-lg border grid grid-cols-1 sm:grid-cols-3 gap-4 items-center relative">
                                                    <div className="sm:col-span-1 flex flex-col items-center justify-center">
                                                        <label htmlFor={`new-guardian-photo-${g.tempId}`} className="block text-xs font-medium text-slate-600 mb-1">Photo</label>
                                                        <div className="w-16 h-16 rounded-full bg-slate-200 flex items-center justify-center border-2 border-dashed border-slate-300 relative overflow-hidden">
                                                            {g.image ? <img src={g.image} className="w-full h-full object-cover" alt="New guardian preview" /> : <CameraIcon className="w-6 h-6 text-slate-500" />}
                                                            <input id={`new-guardian-photo-${g.tempId}`} type="file" accept="image/*" onChange={(e) => handleFileChange(e, g.tempId)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                                                        </div>
                                                    </div>
                                                    <div className="sm:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                        <div>
                                                            <label htmlFor={`new-guardian-firstName-${g.tempId}`} className="block text-sm font-medium text-slate-700">First Name</label>
                                                            <input type="text" id={`new-guardian-firstName-${g.tempId}`} placeholder="e.g., Marcus" value={g.firstName} onChange={e => updateNewGuardian(g.tempId, 'firstName', e.target.value)} className="mt-1 w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 sm:text-sm focus:outline-none focus:ring-sky-500 focus:border-sky-500" />
                                                        </div>
                                                        <div>
                                                            <label htmlFor={`new-guardian-lastName-${g.tempId}`} className="block text-sm font-medium text-slate-700">Last Name</label>
                                                            <input type="text" id={`new-guardian-lastName-${g.tempId}`} placeholder="e.g., Cole" value={g.lastName} onChange={e => updateNewGuardian(g.tempId, 'lastName', e.target.value)} className="mt-1 w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 sm:text-sm focus:outline-none focus:ring-sky-500 focus:border-sky-500" />
                                                        </div>
                                                    </div>
                                                    <button type="button" onClick={() => removeNewGuardian(g.tempId)} className="absolute top-2 right-2 text-slate-400 hover:text-red-600">
                                                        <TrashIcon className="w-4 h-4" />
                                                    </button>
                                                </div>
                                            )}<button type="button" onClick={addNewGuardian} className="w-full flex items-center justify-center space-x-2 px-4 py-2 border-2 border-dashed border-slate-300 rounded-md text-sm font-medium text-slate-600 hover:bg-slate-100 hover:border-slate-400 transition"><PlusIcon /><span>Add Guardian</span></button></div>
                                        </div>
                                    </div>
                                </div>
                            )}
