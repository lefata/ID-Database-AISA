...
import { getPeople, logAccess, getRecentLogs } from '../services/apiService';
...
export const AccessControl: React.FC = () => {
    ...
    const fetchRecentLogs = useCallback(async () => {
        ...
        try {
            const logs = await getRecentLogs(accessToken);
            setRecentLogs(logs);
        } catch (err) {
    ...
    const LogEntry: React.FC<{ log: AccessLog }> = ({ log }) => {
        ...
        return (
            <li className="flex items-center space-x-4 p-3 bg-white rounded-lg shadow-sm">
