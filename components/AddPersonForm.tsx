...
export const AddPersonForm: React.FC<AddPersonFormProps> = ({ onAddPeople, people }) => {
    ...
    const [newGuardians, setNewGuardians] = useState<NewGuardian[]>([]);
    const [siblingSearch, setSiblingSearch] = useState('');
    const [isSaving, setIsSaving] = useState(false);
    const [error, setError] = useState<string | null>(null);

    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, guardianId?: string) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                ...
                if (guardianId) {
                    setNewGuardians(prev => prev.map(g => g.id === guardianId ? { ...g, image: base64String, imagePreview: dataUrl } : g));
                } else {
    ...
    const addNewGuardian = () => {
        setNewGuardians(prev => [...prev, { id: crypto.randomUUID(), firstName: '', lastName: '', image: null, imagePreview: null }]);
    };
    
    const removeNewGuardian = (id: string) => {
        setNewGuardians(prev => prev.filter(g => g.id !== id));
    };

    const updateNewGuardian = (id: string, field: 'firstName' | 'lastName', value: string) => {
        setNewGuardians(prev => prev.map(g => g.id === id ? { ...g, [field]: value } : g));
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Process new guardians first
            for (const guardian of newGuardians) {
                const bio = await generateBio(guardian.firstName, guardian.lastName, PersonCategory.PARENT, 'Parent');
                const newGuardianPerson: Person = {
                    id: crypto.randomUUID(),
                    category: PersonCategory.PARENT,
                    firstName: guardian.firstName,
                    lastName: guardian.lastName,
                    image: guardian.imagePreview!,
                    role: 'Parent/Guardian',
                    bio,
                    googleSheetId: `GS-${Math.floor(10000 + Math.random() * 90000)}`,
                };
                peopleToAdd.push(newGuardianPerson);
                newGuardianIds.push(newGuardianPerson.id);
            }

            // Then process the main person
            const bio = await generateBio(firstName, lastName, category, roleOrClass);
            const allGuardianIds = category === PersonCategory.STUDENT ? [...new Set([...selectedGuardians, ...newGuardianIds])] : undefined;
            ...
            const newPerson: Person = {
                ...
                ...(category === PersonCategory.STUDENT && { class: roleOrClass, guardianIds: allGuardianIds }),
                ...(category === PersonCategory.PARENT && { role: roleOrClass }),
            };
            
            peopleToAdd.unshift(newPerson); // Add student/staff to the beginning of the array

            onAddPeople(peopleToAdd);
            resetForm();
        } catch (err) {
            setError("Failed to generate bio or save profiles. Please try again.");
            console.error(err);
        } finally {
            setIsSaving(false);
        }
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-4">
                                            {newGuardians.map((guardian, index) => (
                                                <div key={guardian.id} className="p-4 bg-slate-50 rounded-lg border grid grid-cols-1 sm:grid-cols-3 gap-4 relative">
                                                    <div className="sm:col-span-1 flex items-center space-x-3">
                                                        <div className="w-16 h-16 rounded-full bg-slate-200 flex items-center justify-center border-2 border-dashed border-slate-300 relative overflow-hidden">
                                                            {guardian.imagePreview ? <img src={guardian.imagePreview} className="w-full h-full object-cover" /> : <CameraIcon className="w-6 h-6 text-slate-500" />}
                                                            <input type="file" accept="image/*" onChange={(e) => handleFileChange(e, guardian.id)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                                                        </div>
                                                    </div>
                                                    <div className="sm:col-span-2 grid grid-cols-2 gap-4">
                                                        <input type="text" placeholder="First Name" value={guardian.firstName} onChange={e => updateNewGuardian(guardian.id, 'firstName', e.target.value)} className="w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-sky-500 focus:border-sky-500 sm:text-sm" />
                                                        <input type="text" placeholder="Last Name" value={guardian.lastName} onChange={e => updateNewGuardian(guardian.id, 'lastName', e.target.value)} className="w-full border border-slate-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-sky-500 focus:border-sky-500 sm:text-sm" />
                                                    </div>
                                                    <button type="button" onClick={() => removeNewGuardian(guardian.id)} className="absolute top-2 right-2 text-slate-400 hover:text-red-600">
